...
from app.db.database import get_users_db   # renamed dependency
...
class AuthenticationService:
    ...
    def _resolve_default_role(self, db: Session) -> RoleRecord | None:
        return (
            db.query(RoleRecord)
            .filter(RoleRecord.name == DEFAULT_ROLE_NAME)
            .first()
        )

    def register_new_user(
        self, db: Session, payload: UserRegistrationPayload
    ) -> PlatformUserRecord:
        if (
            db.query(PlatformUserRecord)
            .filter(PlatformUserRecord.email_address == payload.email_address)
            .first()
        ):
        ...
        if (
            db.query(PlatformUserRecord)
            .filter(PlatformUserRecord.user_handle == payload.user_handle)
            .first()
        ):
...
def get_current_authenticated_user(
    ...
    user = (
        db.query(PlatformUserRecord)
        .filter(PlatformUserRecord.user_id == user_id)
        .first()
    )
    if user is None:
        raise credentials_exception
